...
inf_ts %>% 
  ggplot(aes(x = date)) +
  geom_line(aes(y = pce, color = "PCE"), size = 0.8, alpha = 0.9) +
  geom_line(aes(y = core, color = "CPI Core index"), size = 0.8, alpha = 0.9) +
  geom_line(aes(y = cpi, color = "CPI"), size = 0.8, alpha = 0.9) +
  geom_hline(yintercept = 0, lty = 2, alpha = 0.4) +
  scale_x_yearquarter() +
  labs(x = "",
       y = "% change from a year ago",
       title = "Three different inflation measures",
       subtitle = "US: 01/1960 - 01/2024",
       caption = "Source: US Bureau of Labor Statistics/Bureau of Economic Analysis.") +
